...
export default async function DashboardPage() {
  // 1. نعرف الموظف الحالي.
  // هذه الدالة تتأكد من:
  // Clerk login + Employee record في Neon + status = active.
  const currentEmployee = await requireActiveEmployee();

  // 2. نجلب البيانات المناسبة للدور الحالي.
  // Promise.all يجعل الطلبات المستقلة تعمل معًا بدل الانتظار واحدة وراء الأخرى.
  const [
    employees,
    tickets,
    tasks,
  ...
  // 3. نحسب الإجماليات من البيانات المسموح للمستخدم برؤيتها فقط.
  const totalEmployees = employees.length;
  const totalTickets = tickets.length;
  const totalTasks = tasks.length;
  const totalAnnouncements = announcements.length;
  ...
  const statCards = [
    ...
    },
    {
      title: "Tickets",
      value: totalTickets,
      description: `${openTickets.length} open tickets`,
    },
    {
      title: "Tasks",
      value: totalTasks,
    ...
    },
    {
      ...
      description: `${highPriorityAnnouncements.length} high priority`,
    },
  ];

  // 5. نجهز أرقام الرسم البياني من الـTasks المرئية فقط.
  const taskChartData = [
    {
      status: "Todo",
      count: tasks.filter(
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-muted">
          {currentEmployee.role === "admin"
            ? "Overview of your company workspace"
            : currentEmployee.role === "manager"
              ? "Overview of your department workspace"
