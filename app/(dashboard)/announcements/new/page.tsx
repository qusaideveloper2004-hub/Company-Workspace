...
} from "@/lib/require-active-employee";

export default async function NewAnnouncementPage() {
  // يتأكد أن المستخدم مسجل ومربوط بـEmployee وحالته active.
  const currentEmployee = await requireActiveEmployee();

  // Admin وManager فقط يحق لهما الوصول إلى Form الإنشاء.
  const canCreate = canCreateAnnouncement(
    currentEmployee.role
  );

  // Employee لا يرى الـForm حتى لو كتب الرابط يدويًا.
  if (!canCreate) {
    redirect("/unauthorized");
  }

  // إذا كان مسموحًا، نعرض الـClient Form.
